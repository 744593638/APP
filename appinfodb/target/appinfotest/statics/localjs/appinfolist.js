...
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == 0 ||versionid == null || versionid == ""){
	...
	$.ajax({
		...
		success:function(data){
			...
			if(data.Code === 1){
				...
				}else if(data.resultMsg === "success"){//删除失败
					if("open" === obj.attr("saleSwitch")){
		...
		error:function(data){
			if("open" === obj.attr("saleSwitch")){
				alert("恭喜你，【"+obj.attr("appsoftwarename")+"】的【上架】操作成功");
			}else if("close" === obj.attr("saleSwitch")){
				alert("恭喜你，【"+obj.attr("appsoftwarename")+"】的【下架】操作成功");
			}
		}
	});
};
